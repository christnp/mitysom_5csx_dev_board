...
if { $::TimeQuestInfo(nameofexecutable) == "quartus_sta" } {
  ...
  set has_2x_clk 1
  if { [ get_collection_size [get_clocks {*kernel_pll*1]*divclk}] ] == 0 } {
    set has_2x_clk 0
  }
  set kclk2x [get_clocks {*kernel_pll*1]*divclk}]
  ...
  if { $has_2x_clk } {
    remove_clock {*kernel_pll*1]*divclk}
    create_generated_clock -name {*kernel_pll*1]*divclk} -source [get_node_info -name [ get_nodes {*kernel_pll*[1]*vco?ph[0]} ] ] -multiply_by [ expr $multby * 2] -divide_by $divby -master_clock $refclk [get_pins {*kernel_pll*1]*divclk}]
  }
